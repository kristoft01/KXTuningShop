...
box.innerHTML=cart.map(item=>` <div class="cart-item"> <div> <strong>${esc(item.name)}</strong> <small>${Number(item.price).toFixed(2)} lei / buc.</small> </div>
...
form.innerHTML=` <div class="eyebrow">DATE CLIENT</div> <h3>Finalizează comanda</h3>
...
if(servicesBox){
servicesBox.innerHTML=s.map(x=>`       <article class="card service-card">         <div class="card-icon">✦</div>         <h3>${esc(x.name)}</h3>         <p>${esc(x.description||'')}</p>       </article>
    `).join('');
}
...
s.map(x=>`         <option>${esc(x.name)}</option>
      `).join('');
}
...
projects?.length
?projects.map(x=>`         <article class="project-card">
          ${
            x.image_url
